...
class CustomRulesDB(object):
    ...
    def parse_sheets(self, f, ontology_name, all_tags):
        ...
        for sub_sheet_name, sub_sheet_df in data.items():
            ...
            # save to DB
            coll = self.client[self.custom_rules_db][ontology_name]
    ...
    def get_rules(self, ontology_name, rule_type):
        coll = self.client[self.custom_rules_db][ontology_name]
        for row in coll.find({'_id': rule_type}):
            return row['rules']
        return []

    def apply_exact_rules(self, ontology_name, tags, vendor_tags):
        attrvals = tags_to_attrvals(vendor_tags)
        rules = self.get_rules(ontology_name, self.rule_type_exact)
        out = []
        for rule in rules:
    ...
    def apply_conditional_rules(self, ontology_name, tags, vendor_tags):
        rules = self.get_rules(ontology_name, self.rule_type_conditional)
        out = []
        for rule in rules:
    ...
    def apply_category_based_rules(self, ontology_name, tags):
        rules = self.get_rules(ontology_name, self.rule_type_category_based)
        out = []
        for rule in rules:
    ...
    def apply_rules(self, ontology_name, tags, vendor_tags):
        out = []
        out.extend(self.apply_exact_rules(ontology_name, tags, vendor_tags))
        out.extend(self.apply_conditional_rules(ontology_name, tags, vendor_tags))
        out.extend(self.apply_category_based_rules(ontology_name, tags))
        return out

    def parse_fabric_sheets(self, f, ontology_name, all_tags):
        data = pd.read_excel(f, dtype=str, sheet_name=None, keep_default_na=False)
        all_errors = []
        for sub_sheet_name, sub_sheet_df in data.items():
    ...
    def apply_fabric_rules(self, ontology_name, tags, vendor_tags):
        attrvals = tags_to_attrvals(vendor_tags)
...
def test_non_image(db, ontology, tags, vendor_tags):
    ...
    print(db.apply_rules(ontology, tags, vendor_tags))


def test_fabric(db, ontology, tags):
    # vendor_tags = ['Material:100% Cotton']
    vendor_tags = ["Material:64% Polyester, 34% Viscose and 2% Spandex"]
    print(db.apply_fabric_rules(ontology, tags, vendor_tags))
...
if __name__ == '__main__':
    ...
    # print(parse_material('64% Cotton, 33% Nylon,3% Spandex'))
    print(parse_material('65% Polyester and 35 % Viscose'))
