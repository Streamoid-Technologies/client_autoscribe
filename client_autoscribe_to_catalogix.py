...
def real_time_upload(db, vendor_name, brand_name, style_codes):
    ...
    return get_data(db, data)

def get_brands_sc(db, vendor, brands = None):
    if(brands == None):
...
def get_data(db, data):
    ...
    for brand in data.keys():
        ...
        if store_uuid == -1:
            ...
            continue
        store_settings = get_params(brand)
...
def get_style_codes(vendor, brands, db):
    ...
    for brand in brands:
        logger.info(brand)
        if db.get_new_products(vendor, brand) == []:
            continue
        style_codes[brand] = db.get_new_products(vendor, brand)
...
def reject_catalogix_push(vendor_name, brand_name, data):
    ...
    store_uuid = get_store_uuid(brand_name)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Process stylecodes and ABFRL brands.")
    parser.add_argument("abfrl_brands",default=None, help="ABFRL brands to process",  nargs='*')
    parser.add_argument("--stylecodes",default=None, help="Stylecodes to process")
    

    args = parser.parse_args()
    logger.info(args.stylecodes)
    from client_autoscribe_db_v2 import ClientAutoscribeDB
    db = ClientAutoscribeDB(mongo_host)
    if len(args.abfrl_brands) == 0:
        status, errors = get_brands_sc(db, vendor)
    else:
        if args.stylecodes == None:
            logger.info("NO SC")
            status, errors = get_brands_sc(db ,vendor, args.abfrl_brands)
        else:
            logger.info("SC")
            status, errors = get_style_code_data(db ,vendor, args.abfrl_brands, args.stylecodes)
