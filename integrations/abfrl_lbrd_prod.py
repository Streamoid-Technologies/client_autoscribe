...
class VendorAdapter(BaseAdapter):
    def __init__(self, vendor_config, brand_config):
        ...
        self.title_desc = self.parse_pantaloons(this_file)
    ...
    def parse_pantaloons(self, file_path):
        ...
        for _, row in df.iterrows():
            ...
            product_type = row['Category'].strip()  # row['Product Type'].strip()
            if len(product_type) > 0:
                out.append({'Brand': brand.strip().lower() if brand is not None else None,
                            'Category': product_type.strip().lower() if product_type is not None else None,
                            'Condition': row['Condition'].strip(), 'Department': row['Department'].strip(),
                            'OpeningLine': row['OpeningLine'].strip(), 'StyleTip': row['StyleTip'].strip()})
            else:
                current_brand = None
    ...
    def convert_the_collective_output(self, vendor_name, brand_name, style_code, product, output, data):
        ...
        tshirts_type = data.get('T-shirts Type', None)
        trousers_type = data.get('Trousers Type', None)
        occasion = data.get('Occasion', None)

        '''
        if department == 'men' and category == 'T-shirts' and tshirts_type == 'polo':
            output['brand_attributes']['Product Type'] = 'Polos'
        elif category == 'T-shirts' and tshirts_type == 'regular':
            output['brand_attributes']['Product Type'] = 'T-shirt'
        elif category == 'Jeans':
            output['brand_attributes']['Product Type'] = 'Jeans'
        elif category == 'Trousers' and trousers_type == 'track-pants':
            output['brand_attributes']['Product Type'] = 'Track Pant'
        elif category == 'Shirts' and occasion == 'casual':
            output['brand_attributes']['Product Type'] = 'Casual Shirt'
        '''
    ...
    def convert_output(self, output, **kwargs):
        ...
        myntra_attributes = output.get('myntra_attributes', None)
        if myntra_attributes:
            manufacturer_info = myntra_attributes.get('Manufacturer Name and Address with Pincode', '')
            if manufacturer_info:
                places = ['India', 'Karnataka', 'Mumbai']
                if output['myntra_attributes']['Country Of Origin'] == 'India': #any(place in manufacturer_info for place in places):
                    output['myntra_attributes']['Importer Name and Address with Pincode'] = ''
                else:
                    output['myntra_attributes']['Importer Name and Address with Pincode'] = manufacturer_info
    ...
    def convert_input(self, data, **kwargs):
        ...
        if filename is None:
            return data  # [fix_season_f21(d) for d in data]#data
    ...
    def post_to_vendor(self, output, **kwargs):
        ...
        if r.ok:
            ...
            logger.info(f"VENDOR: abfrl_lbrd_prod, TIME_STAMP: {path}, STYLE_CODE: {style_code}, RESPONSE: {res}")
    ...
    def pantaloons_translations(self, product, curated, translated):
        ...
        title = '%s %s' % (color, title_category)  # '%s %s %s' % (brand, color, title_category)
        ...
        for row in self.title_desc:
            ...
            condition = parse_condition(row['Condition']) if len(row.get('Condition', '')) > 0 else {}
            logger.info('DEBUGPT2: ', condition, row['Category'].lower(), category1.lower(), row['Brand'].lower(),
                        brand.lower(), department.lower(), row['Department'].lower().split(','),
                        matches(tags, condition))
            if row['Category'].lower() == category1.lower() and \
                    row['Brand'].lower() == brand.lower() and \
                    department.lower() in [x.strip() for x in row['Department'].lower().split(',')] and \
                    matches(tags, condition):
        ...
        if not description:
            logger.info(f"NOMATCHPT2: {category1.lower()}, {brand.lower()}, {department.lower()}, {tags}")
