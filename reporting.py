...
from client_autoscribe.client_autoscribe_db import ClientAutoscribeDB, get_db_name
...
class ClientAutoscribeReporting(ClientAutoscribeDB):
    ...
    def style_codes_pushed(self, vendor_name, start_date, end_date):
    ...
    def style_codes_processed(self, vendor_name, start_date, end_date):
        vendor_db = get_db_name(vendor_name)
    ...
    def style_codes_pushed_and_processed(self, vendor_name, start_date, end_date):
        return set(self.style_codes_pushed(vendor_name, start_date, end_date)) & \
               set(self.style_codes_processed(vendor_name, start_date, end_date))

    def analyse_tat(self, vendor_name, start_date, end_date):
        style_codes = self.style_codes_processed(vendor_name, start_date, end_date)
    ...
    def accuracy(self, vendor_name, start_date, end_date):
        out = {}
        out1 = self.range_accuracy(vendor_name, start_date, end_date)
    ...
    def range_accuracy(self, vendor_name, start_date, end_date, style_codes=None):
        # - by streamoid category
        if style_codes is None:
            style_codes = self.style_codes_pushed(vendor_name, start_date, end_date)
        out = {}
        for style_code in style_codes:
            category, wrong, total = self.get_product_data(vendor_name, style_code)
            if category is None:
                continue
    ...
    def top_rejected_attributes(self, vendor_name, start_date, end_date):
        out = {}
        out1, _ = self.range_rejected_attributes(vendor_name, start_date, end_date)
    ...
    def range_rejected_attributes(self, vendor_name, start_date, end_date, style_codes=None):
        # - by marketplace
        if style_codes is None:
            style_codes = self.style_codes_pushed(vendor_name, start_date, end_date)
        out = {}
    ...
    def get_rejected_product(self, vendor_name, style_code):
        vendor_db = get_db_name(vendor_name)
        coll = self.client[vendor_db][self.rejects_coll]
        for row in coll.find({'StyleCode': style_code}).sort('submit_time', ASCENDING):
            return row

    def get_cataloging_product(self, vendor_name, style_code):
        vendor_db = get_db_name(vendor_name)
        coll = self.client[vendor_db][self.cataloging_coll]
        for row in coll.find({'_id': style_code}):
            return row

    def get_product_data(self, vendor_name, style_code):
        ...
        rejections = self.get_rejected_product(vendor_name, style_code)
        ...
        data1 = self.get_cataloging_product(vendor_name, style_code)
        # print(data1['data'])
    ...
    def get_rejection_channels(self, vendor_name):
        vendor_db = get_db_name(vendor_name)
        coll = self.client[vendor_db][self.rejects_coll]
        channels = {}
    ...
    def get_report_data(self, vendor_name, start_date, end_date):
        out = {}
        # Volume report
        processed = list(self.style_codes_processed(vendor_name, start_date, end_date).keys())
        logger.info('%s %s', vendor_name, processed)
        out['pushed'] = len(self.style_codes_pushed(vendor_name, start_date, end_date))
        out['processed'] = len(processed)
        out['pushed_and_processed'] = len(self.style_codes_pushed_and_processed(vendor_name, start_date, end_date))

        # TAT
        tat = {'within_6': 0, 'within_24': 0, 'within_48': 0, 'over_48': 0}
        values = self.analyse_tat(vendor_name, start_date, end_date).values()
        for val in values:
        ...
        # accuracy
        out['accuracy'] = self.range_accuracy(vendor_name, start_date, end_date, processed)

        # rejected
        out['rejected'], out['rejected_raw'] = self.range_rejected_attributes(vendor_name, start_date, end_date,
                                                                              processed)
        return out
    ...
    def create_report(self, start_date, end_date, f):
        vendors = self.get_vendor_names()
        print(vendors)

        data = {}
        for vendor_name in vendors:
            data[vendor_name] = self.get_report_data(vendor_name, start_date, end_date)

        f.write('Volume Report\n\n')
        f.write(',' + ','.join(vendors) + '\n')
        f.write('Style Codes Pushed,')
        f.write(','.join([str(data[v]['pushed']) for v in vendors]) + '\n')
        f.write('Style Codes Processed,')
        f.write(','.join([str(data[v]['processed']) for v in vendors]) + '\n')
        f.write('Overlap of Style Codes Pushed & Processed,')
        f.write(','.join([str(data[v]['pushed_and_processed']) for v in vendors]) + '\n')
        f.write('\n')
        ...
        for msg, field in [('in 6 hours', 'within_6'),
            ...
            self._write_tat_row1(f, msg, field, vendors, data)
            self._write_tat_row2(f, msg, field, vendors, data)
        f.write('\n')
    ...
    def get_vendor_names(self):
        ...
        return list(set(vendor_tokens.keys()) - {'forever21', 'abof', 'people', 'pantaloons', 'skult'})
...
if __name__ == '__main__':
    ...
    with open('report_%s_%s.csv' % (start_date, end_date), 'w') as f:
        rep.create_report(start_date, end_date, f)
